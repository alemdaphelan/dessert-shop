...
class ShoppingCart {
  ...
  addItems(products, index) {
    const currentProduct = products[index];
    const { price } = currentProduct;
    this.total += price;
    const itemIndex = this.items.findIndex(item => item.index === index);
    if (itemIndex > -1) {
      this.items[itemIndex].quantity += 1;
    } else {
      this.items.push(
        
        { ...currentProduct, quantity: 1, index: index });
    }
  }
  removeItems(products, index) {
    const currentProduct = products[index];
    const { price } = currentProduct;
    const itemIndex = this.items.findIndex(item => item.index === index);
...
function App(){
  ...
  const removeFromCart = (index) => {
    ...
    newCart.items = [...cart.items];
    newCart.total = cart.total;
    newCart.removeItems(data, index);
    setCart(newCart);
  }
  ...
  return (
    <div className="main">
      <div className="products-list">
        <h1>Desserts</h1>
        <ProductList data={data} addToCart={addToCart} removeFromCart={removeFromCart} cart={cart} />
      </div>
      <div className="cart">
        <Cart cart={cart} deleteFromCart = {deleteFromCart} data={data} resetFromCart = {resetFromCart} />
      </div>
    </div>
  )
}
...
function ProductList(props) {
  ...
  const handleRemoveFromCart = (index) => {
    ...
    const productQuantity = props.cart.items.find(item => item.index === index)?.quantity || 0;
  };
  return (
    props.data.map(({ image, name, category, price }, index) => {
      const productInCart = props.cart.items.find(item => item.index === index);
      const productQuantity = productInCart ? productInCart.quantity : 0;
  
      return (
        <div className="product" key={index}>
          <img className = {productQuantity===0?"":"selected"} src={`src/${image.desktop}`} alt={name}></img>
          <p className="category">{category}</p>
          <p className="name">{name}</p>
          <p className="price">${price}</p>
          {productQuantity === 0 ? (
            <div className="btn add-to-cart" id="add-to-cart-btn" onClick={() => handleAddToCart(index)}>
            ...
            productQuantity > 0 && (
              <div className="btn minus-or-plus">
                <button className="minus" onClick={() => handleRemoveFromCart(index)}>
                  <svg xmlns="http://www.w3.org/2000/svg" width="10" height="2" fill="#C73B0F" viewBox="0 0 10 2">
                    <path fill="#fff" d="M0 .375h10v1.25H0V.375Z"/>
                  </svg>
                </button>
                <p>{productQuantity}</p>
                <button className="plus" onClick={() => handleAddToCart(index)}>
                  <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10" fill="#C73B0F" viewBox="0 0 10 10">
...
const Cart = (props) => {
  const totalQuantity = props.cart?.items.reduce((total, item) => total + item.quantity, 0);
  const totalPrice = props.cart.total;
  ...
  return (
    <div className={totalQuantity === 0 ? "cart-container" :"cart-container white"}>
      <h2>Your Cart ({totalQuantity})</h2>
      <div className="items-list">
        {totalQuantity>0 ? (<div><div className="list-container">{props.cart.items.map((item, index) => (
          <div key={index} className="cart-item">
            ...
            <p className="price-one">@ ${item.price}</p>
            <p className="price-all">${item.price * item.quantity}</p>
            <button className="delete" onClick={()=>props.deleteFromCart(item.name)}><svg xmlns="http://www.w3.org/2000/svg" width="10" height="10" fill="none" viewBox="0 0 10 10"><path fill="#CAAFA7" d="M8.375 9.375 5 6 1.625 9.375l-1-1L4 5 .625 1.625l1-1L5 4 8.375.625l1 1L6 5l3.375 3.375-1 1Z"/></svg></button>
            </div>
          </div>))}
          </div>
          ...
          <button className="submit-btn" onClick = {()=>handleConfirm()}>Confirm Order</button>
          <Bill handleReset={handleReset} isConfirmed = {isConfirmed} data={props.data} totalQuantity = {totalQuantity} totalPrice = {totalPrice} cart={props.cart}/>
          </div>):(
    ...
    props.isConfirmed ? (
      <div className="bg-dark">
        <div className="bill-container">
          ...
          <p className="bill-confirm">Order Confirmed</p>
          <p className="bill-text">We hope you enjoy our food</p>
          <div className="list-items">
            {props.cart.items.map((item, index) => {
              const indexSrc = props.data.findIndex(items=>items.name===item.name)
              return (<div key={index} className="bill-item">
                <img src={`src/${props.data[indexSrc].image.thumbnail}`}></img>
                ...
                <p className="bill-price-all">${item.price * item.quantity}</p>
              </div>
            )})}
          </div>
          <div className="total-cart">
          ...
          <button className="submit-btn" onClick = {()=>props.handleReset()}>Start New Order</button>
        </div>
      </div>
    ) : null
  );
